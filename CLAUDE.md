...
- **Sections 2, 3, 5, 6, 7 are admin-driven.** They all render `bolim.html` through the `_bolim()` helper in `home/views.py`; content comes from the `Material` model (`section`, `title`, `summary`, `body`, `file`, `muqova`, `link`, `manba`, `order`, `is_published`). To add a section, add a choice to `Material.SECTION_CHOICES` and a thin view calling `_bolim()`. Empty sections render a designed empty state, with an "add material" link for staff. **Add materials as `Material` rows, never as hand-written HTML in `bolim.html`** — the admin-managed flow is the client's explicit choice.
  ...
  - PDF materials (`Material.is_pdf`) get a **"Ko'rish"** button that reveals an inline `<iframe>`; its `src` is set on first click so a 10 MB book is not fetched on page load. One delegated click handler in `bolim.html` covers every card.
...
- **The navbar is one row and sized to the millimetre.** Seven long Uzbek labels + brand + auth barely fit, so the sizes were picked by *measuring* in a real browser, not by eye. The binding case is a **logged-in** user — the username and "Chiqish" button add ~105px that the logged-out view never shows, so always check both. Current headroom: 30px at 1280px viewport, 105px above 1366px. Three things hold it together and move together: `.topbar .wrap`'s `max-width: 1340px` (wider than the 1200px content `--wrap` on purpose), the `1279px` hamburger breakpoint, and `.nav a`'s `font-size: 0.92rem` / `padding: 9px 9px`. Enlarge any of them and the breakpoint has to rise too, or the "Chiqish" button gets clipped by `overflow-x: hidden`.
